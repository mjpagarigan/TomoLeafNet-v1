"""
2_train_phase2_v6.py — Phase 2: Fine-Tuning on Field Dataset (v6)

v6 changes over v5:
  - CRITICAL FIX: Removed preprocess_input() from the data pipeline.
    MobileNetV3Large includes a built-in Rescaling layer that normalizes
    [0,255] → [-1,1] internally. Applying preprocess_input on top causes
    double normalization → near-constant input → model collapse.
    Now feeds raw [0,255] images, matching Flutter's TFLite inference.
  - Dataset: reduced Healthy class (500 → balanced), improved Leaf_Miner
    brightness/contrast for better generalization.
...
Outputs:
    - MODEL/tomoleafnet_v6_final.keras
    - MODEL/tomoleafnet_v6.tflite (DEFAULT optimization)
    - RESULTS/Phase2_Curves_v6.png
    - RESULTS/Phase2_History_v6.csv

Usage:
    python 2_train_phase2_v6.py
"""
...
FINAL_KERAS = os.path.join(BASE_DIR, "MODEL", "tomoleafnet_v6_final.keras")
FINAL_H5 = FINAL_KERAS.replace(".keras", ".h5")
FINAL_TFLITE = os.path.join(BASE_DIR, "MODEL", "tomoleafnet_v6.tflite")
RESULTS_DIR = os.path.join(BASE_DIR, "RESULTS")
CURVES_PATH = os.path.join(RESULTS_DIR, "Phase2_Curves_v6.png")
...
def augment_and_mixup(images, labels):
    """Apply augmentation, extra field augments, and Mixup.

    NO preprocess_input here — the model's built-in Rescaling layer
    handles [0,255] → [-1,1] normalization internally.
    """
...
def train():
    ...
    print("  MobileNetV3Large has built-in Rescaling: [0,255] → [-1,1]")
    print("  Pipeline feeds raw [0,255] — NO preprocess_input applied")
    print("  This matches Flutter TFLite inference (raw pixel values)\n")
    ...
    # Augment + Mixup for training (raw [0,255], no preprocess_input)
    train_ds_aug = train_ds.map(
        augment_and_mixup,
        num_parallel_calls=tf.data.AUTOTUNE,
    )

    # Validation: raw [0,255] — no preprocessing needed, model handles it
    ...
    # Verify Rescaling layer exists in the base model
    ...
    if not rescaling_found:
        print("  ⚠ No Rescaling layer found in first 5 layers — check model architecture")
    ...
    ax1.set_title("Phase 2 v6 — Accuracy (3-Stage Progressive Unfreeze)")
    ...
    ax2.axvline(x=s2_end, color="red", linestyle="--", alpha=0.4,
                label=f"Stage 2->3 (epoch {STAGE1_EPOCHS + STAGE2_EPOCHS})")
    ax2.set_title("Phase 2 v6 — Loss (3-Stage Progressive Unfreeze)")
